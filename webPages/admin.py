...
# RESET VISIT COUNTER
def reset_visit_counts(modeladmin, request):
    modeladmin.model.objects.all().update(visit_count=0)
    modeladmin.message_user(
        request, "Todos los contadores de visitas han sido reseteados"
    )
...
def update_featured_most_visited(modeladmin, request):
    modeladmin.model.objects.update(is_featured=False)
    top_items = modeladmin.model.objects.filter(is_private=False).order_by(
        "-visit_count"
    )[:5]
    ...
    update_featured_most_visited.short_description = (
        "Actualizar destacados - mas visitados"
    )


# UPDATE FEATURED RANDOM
def update_featured_random(modeladmin, request):
    modeladmin.model.objects.update(is_featured=False)
    item_count = modeladmin.model.objects.filter(is_private=False).count()
    count_to_fetch = min(5, item_count)
    if item_count > 0:
        random_items = modeladmin.model.objects.filter(is_private=False).order_by("?")[
            :count_to_fetch
        ]
        ...
        modeladmin.message_user(
            request, f"{count_to_fetch} productos fueron destacados aleatoriamente."
        )
    else:
        modeladmin.message_user(request, "No hay suficientes productos para destacar.")

    update_featured_random.short_description = (
        "Actualizar destacados - selección aleatoria"
    )
